...
import { Sliders, Terminal, BookOpen, ArrowRight, Award, Layers, Zap, Play } from 'lucide-react';
...
export const LandingHero: React.FC<LandingHeroProps> = ({ onNavigate, onOpenAI }) => {
  ...
  const cards = [
    { title: copy.cards.composer.title, text: copy.cards.composer.text, icon: Sliders, tab: 'composer' as const },
    { title: copy.cards.bloch.title, text: copy.cards.bloch.text, icon: Layers, tab: 'bloch' as const },
    { title: copy.cards.code.title, text: copy.cards.code.text, icon: Terminal, tab: 'sandbox' as const },
    { title: copy.cards.curriculum.title, text: copy.cards.curriculum.text, icon: BookOpen, tab: 'curriculum' as const },
    { title: copy.cards.quiz.title, text: copy.cards.quiz.text, icon: Award, tab: 'quiz' as const },
    { title: copy.cards.dashboard.title, text: copy.cards.dashboard.text, icon: Zap, tab: 'dashboard' as const },
  ];
    ...
    <div className="space-y-14 py-8">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 p-7 sm:p-12 lg:p-16 items-center min-h-[610px]">
          <div className="lg:col-span-7 space-y-7">
            ...
            <h1 className="max-w-3xl text-4xl sm:text-6xl lg:text-7xl font-semibold tracking-tight leading-[.98] text-zinc-100">
              {copy.title1}<br />
              <span className="template-accent font-bold">{copy.title2}</span><br />
              {copy.title3}
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => onNavigate('composer')} className="template-button px-5 py-3 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all">
                <Sliders className="w-4 h-4" /> {copy.openComposer}
              </button>
              <button onClick={() => onNavigate('curriculum')} className="px-5 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/15 text-zinc-200 text-sm flex items-center gap-2 transition-all">
                <Play className="w-4 h-4" /> {copy.startLearning}
              </button>
          ...
          <div className="lg:col-span-5">
            <div className="glass-panel rounded-[1.7rem] p-5 sm:p-6 max-w-md mx-auto relative overflow-hidden">
              <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-2">
                <div>
                  <div className="text-[10px] uppercase tracking-[.2em] text-zinc-500">{copy.livePreview}</div>
                  <div className="text-sm font-medium text-zinc-100 mt-1">{copy.singleQubitLab}</div>
                </div>
              ...
              <div className="grid grid-cols-5 gap-2 mt-4">
                {(['I', 'H', 'X', 'Z', 'S'] as const).map((gate) => (
                  <button key={gate} onClick={() => setHeroGate(gate)} className={`py-2 rounded-lg text-xs font-mono border transition-all ${heroGate === gate ? 'bg-[#dfff3f] text-black border-[#dfff3f]' : 'bg-white/5 text-zinc-400 border-white/10 hover:bg-white/10'}`}>
                    {gate}
                  </button>
                ))}
              </div>
      ...
      <section className="space-y-6">
        <div className="max-w-2xl">
          <div className="text-[10px] uppercase tracking-[.22em] text-zinc-500 mb-3">{copy.workspaceTitle}</div>
          <h2 className="text-3xl sm:text-4xl font-semibold text-zinc-100">{copy.workspaceSubtitle}</h2>
        </div>
          ...
          {cards.map((card) => {
            ...
            return (
              <button key={card.title} onClick={() => onNavigate(card.tab)} className="glass-card group text-left p-6 rounded-2xl min-h-48 hover:-translate-y-1 hover:border-white/25 transition-all">
                ...
                <p className="text-sm leading-6 text-zinc-500 mt-2">{card.text}</p>
                <div className="mt-5 flex items-center gap-2 text-xs text-zinc-400 group-hover:text-[#dfff3f] transition-colors">
                  {copy.openWorkspace} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </div>
              </button>
            );
          })}
