...
import { BookOpen, ExternalLink, UserRound, FileText, Newspaper, GraduationCap } from 'lucide-react';
    ...
    <div className="space-y-10 py-7 text-zinc-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-white">
              {isHindi ? 'मुफ़्त ई-बुक्स एवं अध्ययन संसाधन' : 'Free eBooks & Learning Resources'}
            </h2>
            <p className="mt-1 text-sm text-zinc-200">
              {isHindi ? 'QubitLab संसाधन गाइड से शुरुआती से लेकर उन्नत स्तर की अध्ययन सामग्री।' : 'Beginner to advanced resources from the supplied QubitLab resource guide.'}
            </p>
          </div>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          {ebooks.map(([title, level, description, href]) => (
            <article key={title} className="rounded-xl border border-white/10 bg-black/20 p-5 hover:border-[#dfff3f]/30 transition-colors">
              <div className="text-sm uppercase tracking-[.12em] text-zinc-200 font-medium">{level}</div>
              <h3 className="mt-2 text-base font-semibold text-white">{title}</h3>
              <p className="mt-2 text-[15px] leading-7 text-zinc-200">{description}</p>
              <div className="mt-4">
                <ResourceLink href={href}>{isHindi ? 'आधिकारिक संसाधन खोलें' : 'Open official resource'}</ResourceLink>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-white">
              {isHindi ? 'अग्रणी शोधकर्ता एवं प्रत्यक्ष संसाधन' : 'Researchers & Direct Resources'}
            </h2>
            <p className="mt-1 text-sm text-zinc-200">
              {isHindi ? 'प्रोफेसर पृष्ठ, पाठ्यक्रम, नोट्स और शोध केंद्र।' : 'Professor pages, courses, notes and research hubs from the supplied guide.'}
            </p>
          </div>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          {researchers.map(([name, institution, specialty, href, secondaryLabel, secondaryHref]) => (
            <article key={name} className="rounded-xl border border-white/10 bg-black/20 p-5">
              ...
              <div className="mt-1 text-sm uppercase tracking-[.12em] text-zinc-200 font-medium">{institution}</div>
              <p className="mt-3 text-[15px] leading-7 text-zinc-200">{specialty}</p>
              <div className="mt-4 flex flex-wrap gap-4">
                <ResourceLink href={href}>{isHindi ? 'संसाधन खोलें' : 'Open resource'}</ResourceLink>
                {secondaryHref && <ResourceLink href={secondaryHref}>{secondaryLabel}</ResourceLink>}
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-white">
              {isHindi ? 'विश्वविद्यालय व्याख्यान श्रृंखलाएं' : 'University Lecture Series'}
            </h2>
            <p className="mt-1 text-sm text-zinc-200">
              {isHindi ? 'गहन अध्ययन के लिए QubitLab पाठ्यक्रम के साथ इनका उपयोग करें।' : 'Use these alongside the QubitLab curriculum for deeper study.'}
            </p>
          </div>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          {lectureSeries.map(([title, description, href]) => (
            <article key={title} className="rounded-xl border border-white/10 bg-black/20 p-5">
              ...
              <p className="mt-2 text-[15px] leading-7 text-zinc-200">{description}</p>
              <div className="mt-4">
                <ResourceLink href={href}>{isHindi ? 'व्याख्यान खोलें' : 'Open lectures'}</ResourceLink>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-white">
              {isHindi ? 'अनुसंधान एवं संदर्भ सामग्री' : 'Research & Reference'}
            </h2>
            <p className="mt-1 text-sm text-zinc-200">
              {isHindi ? 'मानक संदर्भ पुस्तकें और मूलभूत शोध पत्र।' : 'Research and reference material explicitly linked in the supplied guide.'}
            </p>
          </div>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          {researchAndMedia.filter(([title]) => !title.includes('Quantum Frontiers') && !title.includes('Aaronson')).map(([title, description, href]) => (
            <article key={title} className="rounded-xl border border-white/10 bg-black/20 p-5">
              ...
              <p className="mt-2 text-[15px] leading-7 text-zinc-200">{description}</p>
              <div className="mt-4">
                <ResourceLink href={href}>{isHindi ? 'संदर्भ खोलें' : 'Open reference'}</ResourceLink>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-white">
              {isHindi ? 'लेख, ब्लॉग एवं व्याख्यान' : 'Articles, Blogs & Talks'}
            </h2>
            <p className="mt-1 text-sm text-zinc-200">
              {isHindi ? 'क्वांटम समुदाय से जुड़े रहने के लिए पठनीय सामग्री।' : 'Accessible material for staying connected with the quantum-computing community.'}
            </p>
          </div>
        </div>
        <div className="mt-6 grid gap-3 md:grid-cols-2">
      ...
      <section className="rounded-2xl border border-[#dfff3f]/15 bg-[#dfff3f]/[.03] p-6 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-200">
              {isHindi ? 'संसाधन गाइड में दिए गए क्रम का पालन करें।' : 'Follow the order in the supplied resource guide.'}
            </p>
          </div>
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            [isHindi ? 'चरण 1' : 'Phase 1', isHindi ? 'बुनियादी नींव' : 'Foundation', 'Quantum Computing for the Quantum Curious'],
            ...
            [isHindi ? 'चरण 5' : 'Phase 5', isHindi ? 'AI + क्वांटम' : 'AI + Quantum', 'Quantum Computing from Hopfield Nets'],
            ...
            <div key={phase} className="rounded-xl border border-white/10 bg-black/20 p-4">
              <div className="text-sm uppercase tracking-[.12em] text-zinc-200 font-medium">{phase}</div>
