...
export const CurriculumExplorer: React.FC<CurriculumExplorerProps> = ({
  ...
  const { t, isHindi, localizeModuleTitle, localizeTopicTitle, localizeTopicContent } = useLanguage();
  ...
  const topicFormula = (() => {
    switch (selectedModuleId) {
      ...
      case 'mod-algorithms':
        return isHindi ? 'एल्गोरिदम की गति समस्या की संरचना और मापन परिणाम पर निर्भर करती है' : 'Algorithmic speedups depend on the problem structure and measurement outcome';
      default:
        return null;
    }
  })();
      ...
      <div className="lg:col-span-4 bg-white/[.05] border border-white/10 rounded-xl p-4 shadow-xl backdrop-blur-sm space-y-4 h-fit max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between pb-3 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-zinc-100 font-mono">{copy.title}</h3>
          </div>
        ...
        <div className="space-y-3">
          {modules.map((mod, idx) => {
            ...
            const localizedModTitle = localizeModuleTitle(mod.id, mod.title);
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-zinc-200 font-mono">{localizedModTitle}</h4>
                ...
                {isSelected && (
                  <div className="px-3 pb-3 pt-1 space-y-1 border-t border-slate-900">
                    {mod.submodules.map((sub) => {
                      ...
                      const localizedSubTitle = localizeTopicTitle(sub.id, sub.title);
                      return (
                        <div
                          key={sub.id}
                        ...
                        >
                          <div className="flex items-center gap-2 truncate">
                            ...
                            <span className="truncate">{localizedSubTitle}</span>
                          </div>

                          <span className="text-[10px] font-mono text-zinc-500 shrink-0 ml-2">
                            {sub.durationMinutes}{copy.minutesShort}
                          </span>
                        </div>
                      );
                    })}
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="bg-white/[.05] border border-white/10 rounded-xl p-6 shadow-xl backdrop-blur-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-white/10">
            <div>
              <div className="flex items-center gap-2 text-xs font-mono text-[#dfff3f] mb-1">
                <span>{localizeModuleTitle(currentModule.id, currentModule.title)}</span>
                <span>•</span>
                <span className="capitalize">{currentSubmodule.difficulty}</span>
              </div>
              <h2 className="text-xl font-bold text-white font-mono">
                {localizeTopicTitle(currentSubmodule.id, currentSubmodule.title)}
              </h2>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {currentSubmodule.completed ? copy.completedStatus : copy.markAsDone}
              </button>

              {onAskAIExplain && (
                <button
                  onClick={() => onAskAIExplain(localizeTopicTitle(currentSubmodule.id, currentSubmodule.title))}
          ...
          <div className="p-4 rounded-xl bg-black/70 border border-white/10 text-xs sm:text-sm text-zinc-300 leading-relaxed space-y-3 font-sans">
            <p>{localizeTopicContent(currentSubmodule.id, currentSubmodule.content)}</p>
          ...
          {currentQuizId && onOpenQuiz && (
            <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-xs text-amber-200">
                ...
                <span>{copy.practiceAvailable}</span>
              </div>
              <button
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-zinc-200 flex items-center gap-2 font-mono">
              ...
              {copy.interactivePractice}
            </h3>
            <span className="text-xs text-zinc-400 font-mono">{copy.interactive}</span>
          </div>
